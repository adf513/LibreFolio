...
async def test_list_countries_italian(test_server):
    ...
    async with httpx.AsyncClient() as client:
        await create_user_and_login(client)
        response = await client.get(
            f"{API_BASE}/utilities/countries", params={"language": "it"}, timeout=TIMEOUT
            )
...
async def test_normalize_currency_iso_code(test_server):
    ...
    async with httpx.AsyncClient() as client:
        await create_user_and_login(client)
        response = await client.get(
            f"{API_BASE}/utilities/currencies/normalize",
            params={"name": "USD"}, timeout=TIMEOUT
            )
...
async def test_normalize_currency_passthrough(test_server):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.get(
            f"{API_BASE}/utilities/currencies/normalize",
            params={"name": "xyznotacurrency123"}, timeout=TIMEOUT
            )
...
class TestListCurrenciesEndpoint:
    ...
    async def test_list_currencies_italian(self, test_server):
        ...
        async with httpx.AsyncClient() as client:
            await create_user_and_login(client)
            response = await client.get(
                f"{API_BASE}/utilities/currencies", params={"language": "it"}, timeout=TIMEOUT
                )
            assert response.status_code == 200
            data = response.json()
