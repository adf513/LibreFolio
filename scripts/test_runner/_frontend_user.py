"""Frontend user & broker E2E tests: brokers, multi-user, broker sharing."""
...
def populate_registry(registry: dict) -> None:
    ...
    cat = make_category(
        help_text="Frontend user & broker E2E tests (brokers, multi-user, sharing)",
        description="""Frontend User & Broker Tests\n\nOptions: --ui, --headed, --debug""")
    add_test(cat, "brokers", front_brokers, name="Brokers Tests", desc="CRUD broker, import files modal", prereq="Login working", tests="brokers/brokers.spec.ts")
    ...
    add_test(cat, "all", front_user_all, test_names=False, name="All User Tests", desc="Run all user/broker E2E tests")
