...
<script lang="ts">
    ...
    let optionTypes = $derived.by<ReadonlyArray<TransactionTypeCode>>(() => {
        // Reading $typesVersion makes this reactive to async type loading.
        void $typesVersion;
        if (!isTypesLoaded()) return [];
        if (filterPairOnly) return TX_TYPES.filter((tt) => getTypeRule(tt).requiresPair);
