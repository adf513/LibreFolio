...
<script lang="ts">
    ...
    import type {MarkerType, SignalStyle} from '$lib/charts/signals';
...
<div class="flex items-center gap-1.5">
    ...
    <div class="flex-1 relative min-w-[50px]">
        <button class="w-full h-7 flex items-center cursor-pointer rounded hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors relative" onclick={togglePopover} title={$t('chartSettings.style.lineType')} type="button">
            <svg class="absolute inset-0" height="24" width="100%">
                <line stroke={style.color} stroke-dasharray={style.lineType === 'dashed' ? '8,4' : style.lineType === 'dotted' ? '2,4' : 'none'} stroke-width={style.lineWidth} x1="2%" x2="98%" y1="14" y2="14" />
            </svg>
        </button>
                            ...
                            {#each [1, 2, 3, 4] as w}
                                ...
                                >
                                    <svg width="20" height="10">
                                        <line x1="2" y1="5" x2="18" y2="5" stroke={style.color} stroke-width={w} />
                                    </svg>
                                </button>
                            {/each}
                        </div>
